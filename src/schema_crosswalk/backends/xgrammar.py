"""XGrammar backend (grammar-constrained decoding for local/OSS models). [IMPURE: LLM]"""
...
class XGrammarAdapter:
    """Proposes mappings via XGrammar-constrained decoding on a local model."""

    def __init__(self, model: str, **model_kwargs: Any) -> None:
        if not _AVAILABLE:
            raise BackendNotInstalled("xgrammar", "xgrammar")
        self.model = model
